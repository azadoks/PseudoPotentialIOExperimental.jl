...
function load_upf(io::IO)
    ...
    mesh = RadialMesh(old_upf["radial_grid"], old_upf["radial_grid_derivative"])
    if typeof(mesh) <: UniformMesh
        mesh_type = "linear"
    elseif typeof(mesh) <: LogMeshWithoutZero
        mesh_type = "log_1"
    elseif typeof(mesh) <: LogMeshWithZero
        mesh_type = "log_2"
    end
    mesh_a = mesh.a
    mesh_b = mesh.b
